...
inster into author(id, name, email) values(1, 'hongildong', 'hongildong@naver.com')
(글자는 ''안에다가 넣어야함!)
...
(스키마(information_schema) 안에 있는 테이블(key_column_usage) // 데이터베이스 selected 안되어 있으면 테이블 앞에 데이터 베이스(information_schema.key) 써주기)
